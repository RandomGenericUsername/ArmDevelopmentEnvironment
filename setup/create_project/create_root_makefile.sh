...
for dir in "${MCU_SRC_DIRS[@]}"; do
    ...
    if [[ ${#MCU_SRC_DIRS[@]} -eq 1 ]]; then
        ...
        suffix=""
        create_rule --target "all_test" --command "\$(MAKE) -C ${CORE_DIR}/${dir} all_test"
        ...
        #this is so test is added only once
    ...
    create_rule --target "build${suffix}" --command "\$(MAKE) -C ${CORE_DIR}/${dir} build"
    ...
    build_project_deps+="build${suffix} "
...
if [[ ${#MCU_SRC_DIRS[@]} -gt 1 ]]; then
    ...
    create_rule --target "build" --dependencies "${build_project_deps}"
